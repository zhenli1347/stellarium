...
void convertInfoIni(const QString& dir, std::ostream& s, QString& boundariesType, QString& author, QString& credit, QString& license, QString& cultureId)
{
...
int main(int argc, char** argv)
{
	if (argc != 4)
	{
		std::cerr << "Usage: " << argv[0] << " skyCultureDir outputDir skyCulturePoDir\n";
		return 1;
	}

	const QString inDir = argv[1];
	const QString outDir = argv[2];
	const QString poDir = argv[3];
	...
	QString boundariesType, author, credit, license, cultureId;
	convertInfoIni(inDir, out, boundariesType, author, credit, license, cultureId);
	...
	dLoader.load(inDir, poDir, cultureId, author, credit, license);
	dLoader.dump(outDir);
